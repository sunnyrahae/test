...
/* ─── 회원 정보 입력 화면 ─── */
#s-register { padding: 0 20px; }
.reg-wrap { padding: 32px 0 20px; }
.reg-domain-badge {
...
.reg-title {
  ...
  background: linear-gradient(135deg, #fff, var(--teach));
  -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}
.reg-sub { font-size: 13px; color: var(--muted); margin-bottom: 30px; line-height: 1.6; }
.reg-form { display: flex; flex-direction: column; gap: 0; }
.reg-field { display: flex; flex-direction: column; gap: 7px; margin-bottom: 20px; }
.reg-label { font-size: 12px; font-weight: 700; color: var(--muted); letter-spacing: .08em; }
.reg-input {
  background: var(--card); border: 1.5px solid var(--border); border-radius: 14px;
  padding: 15px 16px; font-size: 15px; color: var(--text);
  font-family: 'Gowun Dodum', sans-serif; outline: none;
  transition: border-color .2s;
}
.reg-input:focus { border-color: rgba(162,155,254,.6); }
.reg-input::placeholder { color: var(--muted); }
.reg-hint { font-size: 11px; color: var(--muted); padding-left: 4px; line-height: 1.5; }
.reg-err { font-size: 12px; color: #ff6b6b; min-height: 18px; padding-left: 4px; margin-bottom: 4px; }
.reg-submit {
  margin-top: 4px; color: #fff;
  background: linear-gradient(135deg, var(--teach), #fd79a8);
}
...
.email-status {
  ...
  color: var(--study); margin-bottom: 12px;
}
.email-status.error {
  background: rgba(255,107,107,.08); border-color: rgba(255,107,107,.25); color: var(--hrd);
}
.email-status.sending {
  background: rgba(162,155,254,.08); border-color: rgba(162,155,254,.25); color: var(--teach);
}
